    ...
    for idx, s in enumerate(starts):
        ...
        if e - s > raw_win_size:
            ...
            if raw_len + (e - s) <= raw_win_size:
                ...
                add_seq_len=cal_repeat_seqlen(seq)
                out_raw_len = len(raw) // 4  #lx_add
                # print(len(raw), add_seq_len)
                ...
                #     continue
                    ...
                    raw, seq, raw_len = init()
                    continue
                elif out_raw_len <= add_seq_len:
                    print(out_raw_len, add_seq_len)
                    #cause ctc_loss = inf
                    raw, seq, raw_len = init()
                    continue
                else:
...
def get_label_raw(fast5_fn, basecall_group, basecall_subgroup, reverse=False):
    # Open file
    # print(basecall_group, basecall_subgroup)
    try:
