...
do
    ...
    # data preprocessing
    python path/to/generate_dataset/inference_data.py -fast5 ${fast5_path} -records_dir ${tmp_records_dir} -raw_len ${signal_window_length} -threads 40
    printf "${p} preprocessing done"

    # caller
    python path/to/multi_gpu_call.py -model ${model_path} -records_dir ${tmp_records_dir} -output ${basecalled_dir}/${p} -half
